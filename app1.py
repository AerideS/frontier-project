...
db = client['drone_data_flask']  # 'drone_data_flask'라는 데이터베이스를 선택

# 새로운 장치 데이터를 추가하거나 기존 장치의 고도를 업데이트하는 함수를 정의
def add_device(device_type, longitude, latitude, altitude):
    collection_name = device_type.lower() + '_data'  # 컬렉션 이름을 장치 타입에 따라 생성..
    collection = db[collection_name]  # 선택한 데이터베이스에서 해당 이름의 컬렉션을 선택..
...
def get_device(device_type, longitude, latitude):
    collection_name = device_type.lower() + '_data'  # 컬렉션 이름을 장치 타입에 따라 생성
    collection = db[collection_name]  # 선택한 데이터베이스에서 해당 이름의 컬렉션을 선택
...
if __name__ == '__main__':  # 스크립트가 단독으로 실행될 때만 아래 코드를 실행
    ...
    add_device('drone', 40.7128, -74.0060, 100)  # 예시파일 New York City의 고도 100m로 설정
